...
First the model is trained on a set of grayscale face images (known identities). 
...
Then the Jacobian algorithm on the covariance matrix where each rotation updates two rows/columns in O(n) time, so a full computation over all not diagonal elements is O(n^3) time with O(n^2) space.
